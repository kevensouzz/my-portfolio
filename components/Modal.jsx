...
export default function Modal({ onClose, message }) {
  return (
    <section className={`fixed z-50 inset-0 bg-black text-accent bg-opacity-75 backdrop-blur-[2px] flex flex-col justify-center items-center`}>
      <div
        className={`w-[450px] h-[125px] overflow-hidden flex flex-col rounded-2xl border-t-4 border-accent bg-opacity-75 max-sm:w-[375px] max-sm:h-[100px] max-[400px]:w-[275px]`}
      >
        <span
          className={`w-full h-3/5 flex items-center justify-center font-semibold text-2xl max-sm:text-xl max-[400px]:text-lg`}
        >
          {message}
        </span>
